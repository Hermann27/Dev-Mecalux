...
Public Class FrmExtractionClient
            ...
            If ContinuTraitement = 6 Then
                ...
                If NbInfosLibre = NbInfosLibreVue Then 'NbInfosLibreVue
                    Try
                        ...
                        lblSne.Text = "Scénario Extraction des Clients"
    ...
    Public Sub VerificateurChampObligatoire(ByVal Entete As Boolean, ByVal Ligne As Boolean)
        OleAdaptaterschema = New OleDbDataAdapter("select * from P_COLONNEST WHERE CodeTbls='ACC' AND Entete=" & Entete & " AND Ligne=" & Ligne & "  ORDER BY ORDRE", OleConnenection)
    ...
    Public Function RecuperationColonneSage(ByVal Entete As Boolean, ByVal Ligne As Boolean) As String
        ...
        If Entete Then
            OleAdaptaterschema = New OleDbDataAdapter("select * from P_COLONNEST WHERE CodeTbls='ACC' AND (Entete=" & Entete & " OR InfosLibre=true) AND Ligne=False ORDER BY ORDRE", OleConnenection)
        Else
            OleAdaptaterschema = New OleDbDataAdapter("select * from P_COLONNEST WHERE CodeTbls='ACC' AND (Ligne=" & Ligne & " OR InfosLibre=true) AND Entete=False ORDER BY ORDRE", OleConnenection)
        End If
        OleSchemaDataset = New DataSet
        OleAdaptaterschema.Fill(OleSchemaDataset)
        OledatableSchema = OleSchemaDataset.Tables(0)
    ...
    Public Sub EstInfosLibre(ByVal Entete As Boolean, ByVal Ligne As Boolean, ByVal InfosLibre As Boolean)
        ...
        Try
            OleAdaptaterschema = New OleDbDataAdapter("select * from P_COLONNEST WHERE CodeTbls='ACC' AND Entete=" & Entete & " AND InfosLibre=" & InfosLibre & " AND Ligne=" & Ligne, OleConnenection)
            OleSchemaDataset = New DataSet
            OleAdaptaterschema.Fill(OleSchemaDataset)
            OledatableSchema = OleSchemaDataset.Tables(0)
            If OledatableSchema.Rows.Count <> 0 Then
            ...
            If ExcelConnect(NomServersql, NomBaseCpta, Nom_Utilsql, Mot_Passql) Then
                If RbARcreation.IsChecked Then
                    If SelectChamp IsNot Nothing Then
                        OleAdaptaterschemaSage = New OleDbDataAdapter("select " & SelectChamp & " FROM F_COMPTET WHERE CT_Type=0 AND CT_DateCreate = CBModification ORDER BY F_COMPTET.CT_NUM ", OleExcelConnect)
                    Else
                        OleAdaptaterschemaSage = New OleDbDataAdapter("select * FROM F_COMPTET WHERE CT_Type=0 AND CT_DateCreate = CBModification ORDER BY F_COMPTET.CT_NUM ", OleExcelConnect)
                    End If
                ...
                ElseIf (RbAR2action.IsChecked) Then
                    If SelectChamp IsNot Nothing Then
                        OleAdaptaterschemaSage = New OleDbDataAdapter("select " & SelectChamp & " FROM F_COMPTET WHERE CT_Type=0 ORDER BY F_COMPTET.CT_NUM", OleExcelConnect)
                    Else
                        OleAdaptaterschemaSage = New OleDbDataAdapter("select * FROM F_COMPTET WHERE CT_Type=0 ORDER BY F_COMPTET.CT_NUM", OleExcelConnect)
                    End If
                End If
                ...
                OleAdaptaterschema = New OleDbDataAdapter("select * from P_COLONNEST WHERE CodeTbls='ACC' AND Entete=True ORDER BY ORDRE ", OleConnenection)
                OleSchemaDataset = New DataSet
                OleAdaptaterschema.Fill(OleSchemaDataset)
                OledatableSchema = OleSchemaDataset.Tables(0)
            End If
    ...
    Public Function EcritureEntete() As Boolean
        ...
        Error_journal = File.AppendText(PathsFileFormatiers & "ACC09" & Strings.Right(DateAndTime.Year(Now), 4) & Format(DateAndTime.Month(Now), "00") & Format(DateAndTime.Day(Now), "00") & Format(DateAndTime.Hour(Now), "00") & Format(DateAndTime.Minute(Now), "00") & Format(DateAndTime.Second(Now), "00") & ".txt")
        Try
            If OledatableSchemaSage.Rows.Count <> 0 Then
                For i As Integer = 0 To OledatableSchemaSage.Rows.Count - 1
                    ListBox.Items.Add("Extraction du Client : <[" & OledatableSchemaSage.Rows(i).Item("CT_INTITULE") & "]> Code du Client : " & OledatableSchemaSage.Rows(i).Item("CT_NUM"))

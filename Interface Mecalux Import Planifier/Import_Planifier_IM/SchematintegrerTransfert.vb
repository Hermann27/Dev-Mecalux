...
Public Class SchematintegrerTransfert
    Private Sub SchematintegrerTransfert_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from WIT_SCHEMA where CheminFormat='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(i).Cells("TypeFormat1").Value) & "' and Cible='" & DataListeIntegrer.Rows(i).Cells("Cible1").Value & "'", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From WIT_SCHEMA where CheminFormat='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(i).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(i).Cells("TypeFormat1").Value) & "' and Cible='" & DataListeIntegrer.Rows(i).Cells("Cible1").Value & "'"
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        DelFile = "Delete From PLANIFICATION where (Intitule='Import Document Transfert') and IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        ...
                        OleAdaptaterEnreg = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT DOCUMENTTRANSFERT'", OleConnenection)
                        ...
                        If OledatableEnreg.Rows.Count <> 0 Then
                            If Directory.Exists(OledatableEnreg.Rows(0).Item("Repertoire")) = True Then
                                DelFile = "Delete From FTPREPERTOIRE where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT DOCUMENTTRANSFERT'"
                                OleCommandDelete = New OleDbCommand(DelFile)
                                OleCommandDelete.Connection = OleConnenection
                                OleCommandDelete.ExecuteNonQuery()
                ...
                For n = 0 To DataListeSchema.RowCount - 1
                    If DataListeSchema.Rows(n).Cells("Cible").Value = "Repertoire" Then
                        If IsNumeric(DataListeSchema.Rows(n).Cells("IDDossier").Value) = True Then
                            OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_SCHEMA WHERE Cible='" & DataListeSchema.Rows(n).Cells("Cible").Value & "' And  CheminFormat='" & DataListeSchema.Rows(n).Cells("Chemin").Value & "' and CheminFilexport='" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "' and Type='" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "'", OleConnenection)
                            OleEnregDataset = New DataSet
                            OleAdaptaterEnreg.Fill(OleEnregDataset)
                            OledatableEnreg = OleEnregDataset.Tables(0)
                            If OledatableEnreg.Rows.Count <> 0 Then
                            Else
                                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_SCHEMA WHERE IDDossier=" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & " ", OleConnenection)
                                OleEnregDataset = New DataSet
                                OleAdaptaterEnreg.Fill(OleEnregDataset)
                                OledatableEnreg = OleEnregDataset.Tables(0)
                                If OledatableEnreg.Rows.Count <> 0 Then
                                Else
                                    If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCial").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCpta").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                        Insertion = "Insert Into WIT_SCHEMA (BaseCial,Cible,BaseCpta,CheminFormat,CheminFilexport,NomFormat,NomFilexport,Type,Deplace,Mode,Feuille_Excel,TriNom,TriCreation,TriModification,IDDossier) VALUES ('" & DataListeSchema.Rows(n).Cells("BaseCial").Value & "','" & DataListeSchema.Rows(n).Cells("Cible").Value & "','" & DataListeSchema.Rows(n).Cells("BaseCpta").Value & "','" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "','" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "'," & DataListeSchema.Rows(n).Cells("Deplace").Value & ",'" & DataListeSchema.Rows(n).Cells("Mode").Value & "','" & DataListeSchema.Rows(n).Cells("Feuille_Excel").Value & "'," & DataListeSchema.Rows(n).Cells("Nom").Value & "," & DataListeSchema.Rows(n).Cells("Création").Value & "," & DataListeSchema.Rows(n).Cells("Modification").Value & "," & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ")"
                                ...
                                Else
                                    If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCial").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCpta").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                        If RetourneDirectoryFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetournePassWordFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneServeurFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneUserFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" Then
                                            ...
                                            OleCommandEnreg.ExecuteNonQuery()
                                            Insert = True
                                            If Directory.Exists(PatchImportftp & "DOCUMENTTRANSFERT" & DataListeSchema.Rows(n).Cells("IDDossier").Value) = False Then
                                                Directory.CreateDirectory(PatchImportftp & "DOCUMENTTRANSFERT" & DataListeSchema.Rows(n).Cells("IDDossier").Value)
                                                Insertion = "Insert Into FTPREPERTOIRE (IDDossier,Traitement,Repertoire) VALUES (" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ",'IMPORT DOCUMENTTRANSFERT','" & PatchImportftp & "DOCUMENTTRANSFERT" & DataListeSchema.Rows(n).Cells("IDDossier").Value & "')"
                                                OleCommandEnreg = New OleDbCommand(Insertion)
                                                OleCommandEnreg.Connection = OleConnenection
                                                OleCommandEnreg.ExecuteNonQuery()
                                            Else
                                                ...
                                                OleAdaptaterFtp = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value)) & " And  Traitement='IMPORT DOCUMENTTRANSFERT'", OleConnenection)
    ...
    Private Sub BT_DelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_DelRow.Click
        ...
        For i As Integer = 0 To DataListeSchema.RowCount - 1
            DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WIT_SCHEMA") + i
        Next i
    End Sub
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WIT_SCHEMA") + i
        If RenvoieID("WIT_SCHEMA") = DataListeSchema.Rows(0).Cells("IDDossier").Value Then

        Else
            For j As Integer = 0 To DataListeSchema.RowCount - 1
                DataListeSchema.Rows(j).Cells("IDDossier").Value = RenvoieID("WIT_SCHEMA") + j
            Next j
        End If
    End Sub
        ...
        Try
            If e.RowIndex >= 0 Then
                If DataListeSchema.Columns(e.ColumnIndex).Name = "Type" Then
                    Idexe = e.RowIndex
                    SelectionFormatTransfert.ShowDialog()
                End If
    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_SCHEMA where  Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(n).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value) & "'", OleConnenection)
                OleEnregDataset = New DataSet
                OleAdaptaterEnreg.Fill(OleEnregDataset)
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    Insertion = "UPDATE WIT_SCHEMA SET Deplace=" & DataListeIntegrer.Rows(n).Cells("Deplace1").Value & ",TriNom=" & DataListeIntegrer.Rows(n).Cells("Nom1").Value & ",TriCreation=" & DataListeIntegrer.Rows(n).Cells("Création1").Value & ",TriModification=" & DataListeIntegrer.Rows(n).Cells("Modification1").Value & " where  Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(n).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value) & "'"
                    OleCommandEnreg = New OleDbCommand(Insertion)
                    OleCommandEnreg.Connection = OleConnenection
                    OleCommandEnreg.ExecuteNonQuery()
                    Insert = True
            ...
            If e.RowIndex >= 0 Then
                If DataListeIntegrer.Columns(e.ColumnIndex).Name = "RechercheFichier1" Then
                    ...
                    ElseIf DataListeIntegrer.Rows(e.RowIndex).Cells("Cible1").Value = "BaseSQL" Then
                        ...
                        '        MsgBox("Connexion à la Base SQL : " & RetourneBaseSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)) & " Réussie !" & Chr(13) & "" & Chr(13) & " La table SQL : " & RetourneTableSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)) & " Existe !")
                        '    Else

...
Public Class InfoLibreTransfert
    ...
    Private Sub InfoLibreTransfert_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub UpdateParametre()
        ...
        If CheckInfo.Checked = True Then
            Try
                If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                    OleUpdatAdaptater = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And Libre=True And Fichier='F_DOCENTETE'", OleConnenection)
                    ...
                    If OleDatable.Rows.Count <> 0 Then
                        UpdateSociete = "Update WIT_COL SET Type='" & Trim(DateFin.Text) & "',Libre=True,InfoLigne=False' where  Libelle='" & Trim(DateDebut.Text) & "' And Libre=True And Fichier='F_DOCENTETE'"
        ...
        Else
            If Infoligne.Checked = True Then
                Try
                    If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                        OleUpdatAdaptater = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And InfoLigne=True And Fichier='F_DOCLIGNE'", OleConnenection)
                        OleUpdatDataset = New DataSet
                        OleUpdatAdaptater.Fill(OleUpdatDataset)
                        OleDatable = OleUpdatDataset.Tables(0)
                        If OleDatable.Rows.Count <> 0 Then
                            UpdateSociete = "Update WIT_COL SET Type='" & Trim(DateFin.Text) & "',Libre=False,InfoLigne=True where  Libelle='" & Trim(DateDebut.Text) & "' And InfoLigne=True And Fichier='F_DOCLIGNE'"
                            OleCommandUpdate = New OleDbCommand(UpdateSociete)
                            OleCommandUpdate.Connection = OleConnenection
                            OleCommandUpdate.ExecuteNonQuery()
                            MsgBox("Modification Effectuée avec Succès!", MsgBoxStyle.Information, "Modification Information libre")
    ...
    Private Sub Creationperiode()
        ...
        If CheckInfo.Checked = True Then
            Try
                If Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "'AND Fichier='F_DOCENTETE'", OleConnenection)
                    ...
                    Else
                        OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "' And Libre=True And Fichier='F_DOCENTETE'", OleConnenection)
                        OleEnregDataset = New DataSet
                        OleAdaptaterEnreg.Fill(OleEnregDataset)
                        OledatableEnreg = OleEnregDataset.Tables(0)
                        If OledatableEnreg.Rows.Count <> 0 Then
                        ...
                        Else
                            Insertion = "Insert Into WIT_COL (ColDispo,Libelle,Libre,Type,InfoLigne,Fichier,Champ) VALUES ('" & Trim(Periode.Text) & "','" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "',True,'" & Trim(DateFin.Text) & "',False,'F_DOCENTETE','" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "')"
        ...
        Else
            If Infoligne.Checked = True Then
                Try
                    If Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                        OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "' AND Fichier='F_DOCLIGNE'", OleConnenection)
                        OleEnregDataset = New DataSet
                        OleAdaptaterEnreg.Fill(OleEnregDataset)
                        OledatableEnreg = OleEnregDataset.Tables(0)
                        If OledatableEnreg.Rows.Count <> 0 Then
                            MsgBox("Impossible! Information utilisée par le descriptif !", MsgBoxStyle.Information, "Creation Information Libre")
                        Else
                            OleAdaptaterEnreg = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "' And InfoLigne=True And Fichier='F_DOCLIGNE'", OleConnenection)
                            OleEnregDataset = New DataSet
                            OleAdaptaterEnreg.Fill(OleEnregDataset)
                            OledatableEnreg = OleEnregDataset.Tables(0)
                            If OledatableEnreg.Rows.Count <> 0 Then
                                MsgBox("Impossible! Information Existante", MsgBoxStyle.Information, "Creation Information Libre")
                            Else
                                Insertion = "Insert Into WIT_COL (ColDispo,Libelle,Libre,Type,InfoLigne,Fichier,Champ) VALUES ('" & Trim(Periode.Text) & "','" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "',False,'" & Trim(DateFin.Text) & "',True,'F_DOCLIGNE','" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "')"
                                OleCommandEnreg = New OleDbCommand(Insertion)
                                OleCommandEnreg.Connection = OleConnenection
                                OleCommandEnreg.ExecuteNonQuery()
                                OleAdaptaterEnreg = New OleDbDataAdapter("select * From S_FICHIER where  Fichier='" & Join(Split(Join(Split(Join(Split(Join(Split(Trim(DateDebut.Text), " "), "_"), "-"), ""), "/"), ""), "\"), "") & "'", OleConnenection)
    ...
    Private Sub supprimeperiode()
        ...
        If CheckInfo.Checked = True Then
            Try
                If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And Libre=True And Fichier='F_DOCENTETE'", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And Libre=True And Fichier='F_DOCENTETE'"
        ...
        Else
            If Infoligne.Checked = True Then
                Try
                    If Trim(DateDebut.Text) <> "" And Trim(Periode.Text) <> "" And Trim(DateFin.Text) <> "" Then
                        OleAdaptaterDelete = New OleDbDataAdapter("select * From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And InfoLigne=True And Fichier='F_DOCLIGNE'", OleConnenection)
                        OleDeleteDataset = New DataSet
                        OleAdaptaterDelete.Fill(OleDeleteDataset)
                        OledatableDelete = OleDeleteDataset.Tables(0)
                        If OledatableDelete.Rows.Count <> 0 Then
                            DelFile = "Delete From WIT_COL where  Libelle='" & Trim(DateDebut.Text) & "' And InfoLigne=True And Fichier='F_DOCLIGNE'"
                            OleCommandDelete = New OleDbCommand(DelFile)
                            OleCommandDelete.Connection = OleConnenection
                            OleCommandDelete.ExecuteNonQuery()
                            MsgBox("Suppression Reussie", MsgBoxStyle.Information, "Suppression Information libre")
    ...
    Private Sub Initialiser()
        Try
            OleSocieteAdaptater = New OleDbDataAdapter("select * From WIT_COL where Libre=True or InfoLigne=True", OleConnenection)

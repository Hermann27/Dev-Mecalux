    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                ...
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    Insertion = "UPDATE WIS_SCHEMA SET Deplace=" & DataListeIntegrer.Rows(n).Cells("Deplace1").Value & ",TriNom=" & DataListeIntegrer.Rows(n).Cells("Nom1").Value & ",TriCreation=" & DataListeIntegrer.Rows(n).Cells("Création1").Value & ",TriModification=" & DataListeIntegrer.Rows(n).Cells("Modification1").Value & " where  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(n).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value) & "' and Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "'"
                    OleCommandEnreg = New OleDbCommand(Insertion)
                    OleCommandEnreg.Connection = OleConnenection
                    OleCommandEnreg.ExecuteNonQuery()
                    Insert = True

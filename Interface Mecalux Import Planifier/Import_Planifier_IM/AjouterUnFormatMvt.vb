...
Public Class AjouterUnFormatMvt
    ...
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        End If
    End Sub
    Private Sub AjouterUnFormatMvt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            For n = 0 To DataListeFormat.RowCount - 1
                If DataListeFormat.Rows(n).Cells("TypeFormat").Value <> "" Then
                    OleAdaptaterFormat = New OleDbDataAdapter("select * from FORMATIMPMOUV where NomFormat='" & Trim(DataListeFormat.Rows(n).Cells("Format").Value) & "' and Type='" & Renvoietypeformat(DataListeFormat.Rows(n).Cells("TypeFormat").Value) & "'", OleConnenection)
                    ...
                    If OledatableFormat.Rows.Count = 0 Then
                        EstCreation = True
                        SaveFormat = "Insert Into FORMATIMPMOUV (Chemin,NomFormat,Type) VALUES ('" & DataListeFormat.Rows(n).Cells("Chemin").Value & "','" & DataListeFormat.Rows(n).Cells("Format").Value & "','" & Renvoietypeformat(DataListeFormat.Rows(n).Cells("TypeFormat").Value) & "')"
                            ...
                            For i = 0 To nodelist.Count - 1
                                If Trim(nodelist.ItemOf(i).Name) = "TypeFormat" Then
                                    If Renvoietypeformat(DataListeFormat.Rows(e.RowIndex).Cells("TypeFormat").Value) = nodelist.ItemOf(i).InnerText Then
                                        ...
                                        DataListeFormat.Rows(e.RowIndex).Cells("Format").Value = Trim(System.IO.Path.GetFileName(Trim(OpenFileFormat.FileName)))
                                    Else

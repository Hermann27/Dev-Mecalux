...
Public Class SchematintegrerCA
    Private Sub SchematintegrerCA_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AfficheSchemasIntegrer()
        ...
        Try
            OleAdaptaterschema = New OleDbDataAdapter("select * from WICA_SCHEMA", OleConnenection)
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from WICA_SCHEMA where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & "", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From WICA_SCHEMA where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        DelFile = "Delete From PLANIFICATION where (Intitule='Import Compte Analytique') and IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        ...
                        OleAdaptaterEnreg = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT COMPTEA'", OleConnenection)
                        ...
                        If OledatableEnreg.Rows.Count <> 0 Then
                            If Directory.Exists(OledatableEnreg.Rows(0).Item("Repertoire")) = True Then
                                DelFile = "Delete From FTPREPERTOIRE where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT COMPTEA'"
                                OleCommandDelete = New OleDbCommand(DelFile)
                                OleCommandDelete.Connection = OleConnenection
                                OleCommandDelete.ExecuteNonQuery()
                ...
                For n = 0 To DataListeSchema.RowCount - 1
                    If DataListeSchema.Rows(n).Cells("Cible").Value = "Repertoire" Then
                        If IsNumeric(DataListeSchema.Rows(n).Cells("IDDossier").Value) = True Then
                            OleAdaptaterEnreg = New OleDbDataAdapter("select * From WICA_SCHEMA WHERE Cible='" & DataListeSchema.Rows(n).Cells("Cible").Value & "' and CheminFormat='" & DataListeSchema.Rows(n).Cells("Chemin").Value & "' and CheminFilexport='" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "' and Type='" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "'", OleConnenection)
                            OleEnregDataset = New DataSet
                            OleAdaptaterEnreg.Fill(OleEnregDataset)
                            OledatableEnreg = OleEnregDataset.Tables(0)
                            If OledatableEnreg.Rows.Count <> 0 Then
                            Else
                                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WICA_SCHEMA WHERE IDDossier=" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & " ", OleConnenection)
                                OleEnregDataset = New DataSet
                                OleAdaptaterEnreg.Fill(OleEnregDataset)
                                OledatableEnreg = OleEnregDataset.Tables(0)
                                If OledatableEnreg.Rows.Count <> 0 Then
                                ...
                                Else
                                    If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                        Insertion = "Insert Into WICA_SCHEMA (BaseCpta,Cible,CheminFormat,CheminFilexport,NomFormat,NomFilexport,Type,Feuille_Excel,Deplace,IDDossier,Mode) VALUES ('" & DataListeSchema.Rows(n).Cells("BaseCpta").Value & "','" & DataListeSchema.Rows(n).Cells("Cible").Value & "','" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "','" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "','" & DataListeSchema.Rows(n).Cells("FeuilleExcel").Value & "'," & DataListeSchema.Rows(n).Cells("Deplace").Value & "," & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ",'" & DataListeSchema.Rows(n).Cells("Mode").Value & "')"
                                    ...
                                    Else
                                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                            If RetourneDirectoryFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetournePassWordFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneServeurFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneUserFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" Then
                                                ...
                                                OleCommandEnreg.ExecuteNonQuery()
                                                Insert = True
                                                If Directory.Exists(PatchImportftp & "COMPTEA" & DataListeSchema.Rows(n).Cells("IDDossier").Value) = False Then
                                                    Directory.CreateDirectory(PatchImportftp & "COMPTEA" & DataListeSchema.Rows(n).Cells("IDDossier").Value)
                                                    Insertion = "Insert Into FTPREPERTOIRE (IDDossier,Traitement,Repertoire) VALUES (" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ",'IMPORT COMPTEA','" & PatchImportftp & "COMPTEA" & DataListeSchema.Rows(n).Cells("IDDossier").Value & "')"
                                                    OleCommandEnreg = New OleDbCommand(Insertion)
                                                    OleCommandEnreg.Connection = OleConnenection
                                                    OleCommandEnreg.ExecuteNonQuery()
                                                Else
                                                    ...
                                                    OleAdaptaterFtp = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value)) & " And  Traitement='IMPORT COMPTEA'", OleConnenection)
    ...
    Private Sub BT_DelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_DelRow.Click
        ...
        For i As Integer = 0 To DataListeSchema.RowCount - 1
            DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WICA_SCHEMA") + i
        Next i
    End Sub
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WICA_SCHEMA") + i
        If RenvoieID("WICA_SCHEMA") = DataListeSchema.Rows(0).Cells("IDDossier").Value Then

        Else
            For j As Integer = 0 To DataListeSchema.RowCount - 1
                DataListeSchema.Rows(j).Cells("IDDossier").Value = RenvoieID("WICA_SCHEMA") + j
            Next j
        End If
    End Sub
        ...
        Try
            If e.RowIndex >= 0 Then
                If DataListeSchema.Columns(e.ColumnIndex).Name = "Type" Then
                    Idexe = e.RowIndex
                    SelectionFormatCompteA.ShowDialog()
                End If
                    ...
                    ElseIf DataListeSchema.Rows(e.RowIndex).Cells("Cible").Value = "BaseSQL" Then
                        If BaseSQLConnexion(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)), RetourneBaseSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)), RetourneUserSQL(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value))), RetournePasseSQL(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)))) = True Then
                            If ExisteTableSQL(RetourneTableSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value))) = True Then
                                SelectionFiltre.Name = "SelectionFiltreCA"
    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WICA_SCHEMA where  IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & "", OleConnenection)
                OleEnregDataset = New DataSet
                OleAdaptaterEnreg.Fill(OleEnregDataset)
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    If DataListeIntegrer.Rows(n).Cells("Deplace1").Value = False Then
                        Insertion = "UPDATE WICA_SCHEMA SET Deplace=False,Feuille_Excel='" & DataListeIntegrer.Rows(n).Cells("FeuilleExcel1").Value & "' where IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & ""
                        OleCommandEnreg = New OleDbCommand(Insertion)
                        OleCommandEnreg.Connection = OleConnenection
                        OleCommandEnreg.ExecuteNonQuery()
                        Insert = True
                    Else
                        Insertion = "UPDATE WICA_SCHEMA SET Deplace=True,Feuille_Excel='" & DataListeIntegrer.Rows(n).Cells("FeuilleExcel1").Value & "' where  IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & ""
                        OleCommandEnreg = New OleDbCommand(Insertion)
                        OleCommandEnreg.Connection = OleConnenection
                        OleCommandEnreg.ExecuteNonQuery()
                        Insert = True
                    ...
                    ElseIf DataListeIntegrer.Rows(e.RowIndex).Cells("Cible1").Value = "BaseSQL" Then
                        If BaseSQLConnexion(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)), RetourneBaseSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)), RetourneUserSQL(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value))), RetournePasseSQL(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)))) = True Then
                            If ExisteTableSQL(RetourneTableSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value))) = True Then
                                SelectionFiltre.Name = "SelectionFiltreCA1"

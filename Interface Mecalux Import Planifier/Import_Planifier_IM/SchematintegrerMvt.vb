                    ...
                    If DataListeSchema.Rows(n).Cells("Cible").Value = "Repertoire" Then
                        If IsNumeric(DataListeSchema.Rows(n).Cells("IDDossier").Value) = True Then
                            If Trim(DataListeSchema.Rows(n).Cells("Categorie").Value) <> "" Then
                                ...
                                Else
                                    ...
                                    Else
                                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCial").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCpta").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                            Insertion = "Insert Into SCHEMAS_IMPMOUV (Cible,BaseCial,BaseCpta,CheminFormat,CheminFilexport,NomFormat,NomFilexport,Type,Deplace,Mode,Feuille_Excel,TriNom,TriCreation,TriModification,IDDossier,Categorie) VALUES ('" & Trim(DataListeSchema.Rows(n).Cells("Cible").Value) & "','" & DataListeSchema.Rows(n).Cells("BaseCial").Value & "','" & DataListeSchema.Rows(n).Cells("BaseCpta").Value & "','" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "','" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "'," & DataListeSchema.Rows(n).Cells("Deplace").Value & ",'" & DataListeSchema.Rows(n).Cells("Mode").Value & "','" & DataListeSchema.Rows(n).Cells("Feuille_Excel").Value & "'," & DataListeSchema.Rows(n).Cells("Nom").Value & "," & DataListeSchema.Rows(n).Cells("Création").Value & "," & DataListeSchema.Rows(n).Cells("Modification").Value & "," & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ",'" & Trim(DataListeSchema.Rows(n).Cells("Categorie").Value) & "')"
                                ...
                                End If
                            Else
                                MsgBox("La Catégorie Vente/Achat : " & DataListeSchema.Rows(n).Cells("Categorie").Value & " n'est pas renseignée !", MsgBoxStyle.Information, "Insertion des Schemas d'integration")
                            End If
                        Else
    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                ...
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    Insertion = "UPDATE SCHEMAS_IMPMOUV SET Feuille_Excel='" & Join(Split(DataListeIntegrer.Rows(n).Cells("Feuille_Excel1").Value, "'"), "''") & "',Deplace=" & DataListeIntegrer.Rows(n).Cells("Deplace1").Value & ",TriNom=" & DataListeIntegrer.Rows(n).Cells("Nom1").Value & ",TriCreation=" & DataListeIntegrer.Rows(n).Cells("Création1").Value & ",TriModification=" & DataListeIntegrer.Rows(n).Cells("Modification1").Value & " where IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & ""
                    OleCommandEnreg = New OleDbCommand(Insertion)
                    OleCommandEnreg.Connection = OleConnenection
                    OleCommandEnreg.ExecuteNonQuery()
                    Insert = True

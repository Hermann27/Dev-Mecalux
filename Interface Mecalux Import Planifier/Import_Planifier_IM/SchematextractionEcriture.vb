...
Public Class SchematextractionEcriture
    Public FlagConnection As OleDbConnection
    Private Sub SchematextractionEcriture_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub AfficheSchemasIntegrer()
        ...
        Try
            OleAdaptaterschema = New OleDbDataAdapter("select * from WEE_SCHEMA", OleConnenection)
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from WEE_SCHEMA where Cible='" & DataListeIntegrer.Rows(i).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and BaseCpta='" & DataListeIntegrer.Rows(i).Cells("BaseCpta1").Value & "'  and Type='" & DataListeIntegrer.Rows(i).Cells("TypeFormat1").Value & "'", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From WEE_SCHEMA where Cible='" & DataListeIntegrer.Rows(i).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(i).Cells("CheminForma").Value & "' and BaseCpta='" & DataListeIntegrer.Rows(i).Cells("BaseCpta1").Value & "'  and Type='" & DataListeIntegrer.Rows(i).Cells("TypeFormat1").Value & "'"
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        DelFile = "Delete From PLANIFICATION where (Intitule='Export Ecriture') and IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                    End If
                ...
                For n = 0 To DataListeSchema.RowCount - 1
                    If DataListeSchema.Rows(n).Cells("Cible").Value = "Repertoire" Then
                        If IsNumeric(DataListeSchema.Rows(n).Cells("IDDossier").Value) = True Then
                            ...
                            Else
                                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WEE_SCHEMA WHERE IDDossier=" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & "", OleConnenection)
                                OleEnregDataset = New DataSet
                                OleAdaptaterEnreg.Fill(OleEnregDataset)
                                OledatableEnreg = OleEnregDataset.Tables(0)
                                If OledatableEnreg.Rows.Count <> 0 Then
                                ...
                                Else
                                    If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("BaseCpta").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                        Insertion = "Insert Into WEE_SCHEMA (BaseCpta,Cible,CheminFormat,NomFormat,Type,Feuille_Excel,Chmpflag,Valflag,CheminExport,EstEntete,IDDossier) VALUES ('" & DataListeSchema.Rows(n).Cells("BaseCpta").Value & "','" & DataListeSchema.Rows(n).Cells("Cible").Value & "','" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("TypeFormat").Value & "','" & DataListeSchema.Rows(n).Cells("FeuilleExcel").Value & "','" & DataListeSchema.Rows(n).Cells("Flag").Value & "','" & DataListeSchema.Rows(n).Cells("Valeur").Value & "','" & DataListeSchema.Rows(n).Cells("RepExtraction").Value & "'," & DataListeSchema.Rows(n).Cells("EstEntete").Value & "," & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ")"
                                        OleCommandEnreg = New OleDbCommand(Insertion)
                                        OleCommandEnreg.Connection = OleConnenection
                                        OleCommandEnreg.ExecuteNonQuery()
                                        TranscodageIDExiste(CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value), "Ecritures")
                                        Insert = True
                                    End If
                                End If
                            End If
    ...
    Private Sub BT_DelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_DelRow.Click
        ...
        For i As Integer = 0 To DataListeSchema.RowCount - 1
            DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WEE_SCHEMA") + i
        Next i
    End Sub
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("WEE_SCHEMA") + i
        If RenvoieID("WEE_SCHEMA") = DataListeSchema.Rows(0).Cells("IDDossier").Value Then

        Else
            For j As Integer = 0 To DataListeSchema.RowCount - 1
                DataListeSchema.Rows(j).Cells("IDDossier").Value = RenvoieID("WEE_SCHEMA") + j
            Next j
        End If
    End Sub
        ...
        Try
            If e.RowIndex >= 0 Then
                ...
                If DataListeSchema.Columns(e.ColumnIndex).Name = "Type" Then
                    Idexe = e.RowIndex
                    SelectionFormatEcriture.Name = "EXTRACTION"
    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                OleAdaptaterEnreg = New OleDbDataAdapter("select * From WEE_SCHEMA where Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and BaseCpta='" & DataListeIntegrer.Rows(n).Cells("BaseCpta1").Value & "' and Type='" & DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value & "'", OleConnenection)
                OleEnregDataset = New DataSet
                OleAdaptaterEnreg.Fill(OleEnregDataset)
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    Insertion = "UPDATE WEE_SCHEMA SET Feuille_Excel='" & DataListeIntegrer.Rows(n).Cells("FeuilleExcel1").Value & "',Valflag='" & DataListeIntegrer.Rows(n).Cells("Valeur1").Value & "',CheminExport='" & DataListeIntegrer.Rows(n).Cells("RepExtraction1").Value & "',EstEntete=" & DataListeIntegrer.Rows(n).Cells("EstEntete1").Value & " where  Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and BaseCpta='" & DataListeIntegrer.Rows(n).Cells("BaseCpta1").Value & "' and Type='" & DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value & "'"
                    OleCommandEnreg = New OleDbCommand(Insertion)
                    OleCommandEnreg.Connection = OleConnenection
                    OleCommandEnreg.ExecuteNonQuery()
                    Insert = True
        ...
        If e.RowIndex >= 0 Then
            ...
            If OledatableEnreg.Rows.Count <> 0 Then
                If SociéteConnection(OledatableEnreg.Rows(0).Item("Serveur"), OledatableEnreg.Rows(0).Item("BaseDonnee"), OledatableEnreg.Rows(0).Item("NomUser"), OledatableEnreg.Rows(0).Item("MotPas")) = True Then
                    OleAdaptaterEnreg = New OleDbDataAdapter("select * From cbSysLibre where  CB_File='F_ECRITUREC'", FlagConnection)
                    OleEnregDataset = New DataSet
                    OleAdaptaterEnreg.Fill(OleEnregDataset)
                    OledatableEnreg = OleEnregDataset.Tables(0)
                    If OledatableEnreg.Rows.Count <> 0 Then

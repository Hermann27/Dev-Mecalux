...
Public Class FormatintegrationStock
    ...
    Private Sub AfficheColDispo(ByRef Fichier As String)
        ...
        End If
        Try
            OleAdaptater = New OleDbDataAdapter("select * from WIS_COL  Where Fichier='" & Trim(Documentvente) & "' Order BY ColDispo,Libelle", OleConnenection)
        ...
        Try
            If SaveFileXml.ShowDialog = Windows.Forms.DialogResult.OK Then
                If SaveFileXml.FileName <> "" Then
                    OleAdaptaterFormat = New OleDbDataAdapter("select * from WIS_FORMAT where (NomFormat='" & Trim(System.IO.Path.GetFileName(SaveFileXml.FileName)) & "')", OleConnenection)
                    ...
                    If OledatableFormat.Rows.Count = 0 Then
                        ...
                        Filexml.WriteStartElement("FORMAT_IMPORT_STK")
                        ...
                        Filexml.WriteEndElement()
                        Filexml.Close()
                        OleCommandSave = New OleDbCommand("Insert Into WIS_FORMAT (Chemin,NomFormat,Type) VALUES ('" & SaveFileXml.FileName & "','" & Trim(System.IO.Path.GetFileName(SaveFileXml.FileName)) & "','" & Renvoietypeformat(Trim(Txtype.Text)) & "')")
                    ...
                    Else
                        If OledatableFormat.Rows.Count <> 0 And SaveFileXml.FileName = OledatableFormat.Rows(0).Item("Chemin") Then
                            ...
                            Filexml.WriteEndElement()
                            Filexml.Close()
                            OleCommandSave = New OleDbCommand("UPDATE  WIS_FORMAT SET Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "' where (NomFormat='" & System.IO.Path.GetFileName(SaveFileXml.FileName) & "')")
                            OleCommandSave.Connection = OleConnenection
                            OleCommandSave.ExecuteNonQuery()
                            OleCommandSave = New OleDbCommand("UPDATE  WIS_SCHEMA SET Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "',Mode='" & Trim(CbMod.Text) & "' where (NomFormat='" & System.IO.Path.GetFileName(SaveFileXml.FileName) & "')")
                            OleCommandSave.Connection = OleConnenection
                            OleCommandSave.ExecuteNonQuery()
    ...
    Private Sub AfficheFormat()
        ...
        Try
            ...
            OleAdaptaterAfficheFormat = New OleDbDataAdapter("select * from WIS_FORMAT WHERE Type <> 'Longueur Fixe'", OleConnenection)
    ...
    Private Sub DeleteFormat()
        ...
        Try
            If File.Exists(Txt_Chemin.Text) = True Then
                DelFormat = "Delete * from WIS_FORMAT where Chemin='" & Txt_Chemin.Text & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'"
                ...
                OleAdaptaterDeleteFormat = New OleDbDataAdapter("select * from WIS_SCHEMA where CheminFormat='" & Txt_Chemin.Text & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'", OleConnenection)
                ...
                If OledatableDeleteFormat.Rows.Count <> 0 Then
                    For i = 0 To OledatableDeleteFormat.Rows.Count - 1
                        Delschema = "delete from WIS_SCHEMA where CheminFormat='" & Txt_Chemin.Text & "' and CheminFilexport='" & OledatableDeleteFormat.Rows(i).Item("CheminFilexport") & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'"
            ...
            If Trim(Txt_Chemin.Text) <> "" Then
                If File.Exists(Txt_Chemin.Text) = True Then
                    ...
                    While (FileXml.Read())
                        ...
                        End If
                    End While
                    Me.Text = "Formats d'integration des documents de Stock " & Afficheauuser(typedeformat)
    ...
    Private Sub DeleteColDispo()
        ...
        Try
            ...
            If Colbool = False Then
                OleAdaptaterDeleteDispo = New OleDbDataAdapter("select * from WIS_COL where ColDispo='" & DataDispo.Rows(CellCol).Cells("ColDispos").Value & "'", OleConnenection)
    ...
    Private Sub DeleteColSelect()
        ...
        End Try
    End Sub
    Private Sub FormatintegrationStock_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub Cmb_Format_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmb_Format.SelectedIndexChanged
        ...
        Try
            OleAdaptaterCmb = New OleDbDataAdapter("select * from WIS_FORMAT where NomFormat='" & Cmb_Format.Items.Item(Cmb_Format.SelectedIndex) & "'", OleConnenection)
    ...
    Private Sub Cmb_Format_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Cmb_Format.TextChanged
        ...
        Dim OledatableCmb As DataTable
        Try
            OleAdaptaterCmb = New OleDbDataAdapter("select * from WIS_FORMAT where (NomFormat='" & Trim(Cmb_Format.Text) & "')", OleConnenection)
            OleCmbDataset = New DataSet
            OleAdaptaterCmb.Fill(OleCmbDataset)
            OledatableCmb = OleCmbDataset.Tables(0)
            If OledatableCmb.Rows.Count <> 0 Then

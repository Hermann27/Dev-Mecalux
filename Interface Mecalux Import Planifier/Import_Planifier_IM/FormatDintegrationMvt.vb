...
Public Class FormatDintegrationMvt
    ...
    Private Sub AfficheColDispo(ByRef Fichier As String)
        ...
        End If
        Try
            OleAdaptater = New OleDbDataAdapter("select * from COLIMPMOUV  Where Fichier='" & Trim(Documentvente) & "' Order BY ColDispo,Libelle", OleConnenection)
        ...
        Try
            If SaveFileXml.ShowDialog = Windows.Forms.DialogResult.OK Then
                If SaveFileXml.FileName <> "" Then
                    OleAdaptaterFormat = New OleDbDataAdapter("select * from FORMATIMPMOUV where (NomFormat='" & Trim(System.IO.Path.GetFileName(Trim(SaveFileXml.FileName))) & "')", OleConnenection)
                    ...
                    If OledatableFormat.Rows.Count = 0 Then
                        ...
                        Filexml.WriteStartElement("FORMAT_IMPORT_MVT")
                        For j = 0 To DataSelect.RowCount - 1
                            ...
                            If DataSelect.Rows(j).Cells("Cmde").Value = True Then
                                Filexml.WriteStartElement("PieceCmde")
                                Filexml.WriteString("oui")
                                Filexml.WriteEndElement()
                            Else
                                Filexml.WriteStartElement("PieceCmde")
                        ...
                        Filexml.WriteEndElement()
                        Filexml.Close()
                        SaveFormat = "Insert Into FORMATIMPMOUV (Chemin,NomFormat,Type) VALUES ('" & SaveFileXml.FileName & "','" & Trim(System.IO.Path.GetFileName(Trim(SaveFileXml.FileName))) & "','" & Renvoietypeformat(Trim(Txtype.Text)) & "')"
                    ...
                    Else
                        If OledatableFormat.Rows.Count <> 0 And SaveFileXml.FileName = OledatableFormat.Rows(0).Item("Chemin") Then
                            ...
                            Filexml.WriteEndElement()
                            Filexml.Close()
                            SaveFormat = "UPDATE  FORMATIMPMOUV SET Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "' where (NomFormat='" & Trim(System.IO.Path.GetFileName(Trim(SaveFileXml.FileName))) & "')"
                            OleCommandSave = New OleDbCommand(SaveFormat)
                            OleCommandSave.Connection = OleConnenection
                            OleCommandSave.ExecuteNonQuery()
                            SaveFormat = "UPDATE  SCHEMAS_IMPMOUV SET Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "',Mode='" & Trim(CbMod.Text) & "' where (NomFormat='" & Trim(System.IO.Path.GetFileName(Trim(SaveFileXml.FileName))) & "')"
                            OleCommandSave = New OleDbCommand(SaveFormat)
                            OleCommandSave.Connection = OleConnenection
                            OleCommandSave.ExecuteNonQuery()

                        Else
                            MsgBox("Le Fichier " & System.IO.Path.GetFileName(Trim(SaveFileXml.FileName)) & " existe déja! Duplication Impossible", MsgBoxStyle.Information, "Format d'integration")
    ...
    Private Sub AfficheFormat()
        ...
        Try
            ...
            OleAdaptaterAfficheFormat = New OleDbDataAdapter("select * from FORMATIMPMOUV WHERE Type <> 'Longueur Fixe' And Type<>'Table'", OleConnenection)
    ...
    Private Sub DeleteFormat()
        ...
        Try
            If File.Exists(Txt_Chemin.Text) = True Then
                DelFormat = "Delete * from FORMATIMPMOUV where Chemin='" & Txt_Chemin.Text & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'"
                ...
                OleAdaptaterDeleteFormat = New OleDbDataAdapter("select * from SCHEMAS_IMPMOUV where CheminFormat='" & Txt_Chemin.Text & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'", OleConnenection)
                ...
                If OledatableDeleteFormat.Rows.Count <> 0 Then
                    For i = 0 To OledatableDeleteFormat.Rows.Count - 1
                        Delschema = "delete from SCHEMAS_IMPMOUV where CheminFormat='" & Txt_Chemin.Text & "' and CheminFilexport='" & OledatableDeleteFormat.Rows(i).Item("CheminFilexport") & "' And Type='" & Renvoietypeformat(Trim(Txtype.Text)) & "'"
    ...
    Private Sub AffichFormatModifiable()
        ...
        Dim Decal, PieceAuto, PieceCmde, LigneArticle As Object
        Dim i As Integer
                    ...
                    While (FileXml.Read())
                        If FileXml.LocalName = "ColUse" Then
                            ...
                            If (NomColonne <> "" And NomEntete <> "") Then
                                ...
                                If PieceCmde = "oui" Then
                                    DataSelect.Rows(i).Cells("Cmde").Value = True
                                Else
                                    DataSelect.Rows(i).Cells("Cmde").Value = False
                                End If
                        ...
                        End If
                    End While
                    Me.Text = "Formats d'integration des documents " & Afficheauuser(typedeformat)
    ...
    Private Sub MonterLigne()
        Try
            If Index >= 1 Then
                DataSelect.Rows.Insert(Index - 1, DataSelect.Rows(Index).Cells("Selection").Value, DataSelect.Rows(Index).Cells("Position").Value, DataSelect.Rows(Index).Cells("Infos").Value, DataSelect.Rows(Index).Cells("Fichier").Value, DataSelect.Rows(Index).Cells("Piece").Value, DataSelect.Rows(Index).Cells("Cmde").Value, DataSelect.Rows(Index).Cells("Article").Value, DataSelect.Rows(Index).Cells("Defauts").Value, DataSelect.Rows(Index).Cells("ValeurDefauts").Value, DataSelect.Rows(Index).Cells("Libelles").Value)
    ...
    Private Sub DescendreLigne()
        Try
            If Index < DataSelect.RowCount - 1 Then
                DataSelect.Rows.Insert(Index, DataSelect.Rows(Index + 1).Cells("Selection").Value, DataSelect.Rows(Index + 1).Cells("Position").Value, DataSelect.Rows(Index + 1).Cells("Infos").Value, DataSelect.Rows(Index + 1).Cells("Fichier").Value, DataSelect.Rows(Index + 1).Cells("Piece").Value, DataSelect.Rows(Index + 1).Cells("Cmde").Value, DataSelect.Rows(Index + 1).Cells("Article").Value, DataSelect.Rows(Index + 1).Cells("Defauts").Value, DataSelect.Rows(Index + 1).Cells("ValeurDefauts").Value, DataSelect.Rows(Index + 1).Cells("Libelles").Value)
    ...
    Private Sub DeleteColDispo()
        ...
        Try
            ...
            If Colbool = False Then
                OleAdaptaterDeleteDispo = New OleDbDataAdapter("select * from COLIMPMOUV where ColDispo='" & DataDispo.Rows(CellCol).Cells("ColDispos").Value & "'", OleConnenection)
                ...
                If OledatableDeleteDispo.Rows.Count <> 0 Then
                    If OledatableDeleteDispo.Rows(0).Item("Libre") = True Then
                        DataSelect.Rows.Add(DataDispo.Rows(CellCol).Cells("ColDispos").Value, Nothing, True, DataDispo.Rows(CellCol).Cells("Fiche").Value, Nothing, Nothing, Nothing, Nothing, Nothing, DataDispo.Rows(CellCol).Cells("LibelleDispos").Value)
                    Else
                        If OledatableDeleteDispo.Rows(0).Item("InfoLigne") = True Then
                            DataSelect.Rows.Add(DataDispo.Rows(CellCol).Cells("ColDispos").Value, Nothing, True, DataDispo.Rows(CellCol).Cells("Fiche").Value, Nothing, Nothing, Nothing, Nothing, Nothing, DataDispo.Rows(CellCol).Cells("LibelleDispos").Value)
                        Else
                            DataSelect.Rows.Add(DataDispo.Rows(CellCol).Cells("ColDispos").Value, Nothing, False, DataDispo.Rows(CellCol).Cells("Fiche").Value, Nothing, Nothing, Nothing, Nothing, Nothing, DataDispo.Rows(CellCol).Cells("LibelleDispos").Value)
                        End If
                    End If
                End If
            End If
    ...
    Private Sub DeleteColSelect()
        ...
        End Try
    End Sub
    Private Sub FormatDintegrationMvt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub Cmb_Format_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmb_Format.SelectedIndexChanged
        ...
        Try
            OleAdaptaterCmb = New OleDbDataAdapter("select * from FORMATIMPMOUV where NomFormat='" & Cmb_Format.Items.Item(Cmb_Format.SelectedIndex) & "'", OleConnenection)
    ...
    Private Sub Cmb_Format_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Cmb_Format.TextChanged
        ...
        Dim OledatableCmb As DataTable
        Try
            OleAdaptaterCmb = New OleDbDataAdapter("select * from FORMATIMPMOUV where (NomFormat='" & Trim(Cmb_Format.Text) & "')", OleConnenection)
            OleCmbDataset = New DataSet
            OleAdaptaterCmb.Fill(OleCmbDataset)
            OledatableCmb = OleCmbDataset.Tables(0)
            If OledatableCmb.Rows.Count <> 0 Then
    ...
    Private Sub DataSelect_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataSelect.CellContentClick
        If e.RowIndex >= 0 Then
            If DataSelect.Columns(e.ColumnIndex).Name = "Piece" Then
                ...
                Next i
            End If
            If DataSelect.Columns(e.ColumnIndex).Name = "Cmde" Then
                Dim i As Integer
                For i = 0 To DataSelect.RowCount - 1
                    DataSelect.Rows(i).Cells("Cmde").Value = False
                Next i
                DataSelect.UpdateCellValue(e.ColumnIndex, e.RowIndex)
                DataSelect.EndEdit()
                If DataSelect.Rows(e.RowIndex).Cells("Cmde").Value = True Then

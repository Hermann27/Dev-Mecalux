...
Public Class AFormatTransfert
    ...
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        End If
    End Sub
    Private Sub AFormatTransfert_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            For n = 0 To DataListeFormat.RowCount - 1
                If DataListeFormat.Rows(n).Cells("TypeFormat").Value <> "" Then
                    OleAdaptaterFormat = New OleDbDataAdapter("select * from WIT_FORMAT where NomFormat='" & Trim(DataListeFormat.Rows(n).Cells("Format").Value) & "' and Type='" & Renvoietypeformat(DataListeFormat.Rows(n).Cells("TypeFormat").Value) & "'", OleConnenection)
                    ...
                    If OledatableFormat.Rows.Count = 0 Then
                        EstCreation = True
                        SaveFormat = "Insert Into WIT_FORMAT (Chemin,NomFormat,Type) VALUES ('" & DataListeFormat.Rows(n).Cells("Chemin").Value & "','" & DataListeFormat.Rows(n).Cells("Format").Value & "','" & Renvoietypeformat(DataListeFormat.Rows(n).Cells("TypeFormat").Value) & "')"

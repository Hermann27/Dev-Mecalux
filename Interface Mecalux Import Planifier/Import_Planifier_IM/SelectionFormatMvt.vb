...
Public Class SelectionFormatMvt
    ...
    Private Sub AfficheSchemasIntegration()
        Try
            ...
            OleAdaptaterschema = New OleDbDataAdapter("select * from FORMATIMPMOUV ORDER BY NomFormat ASC", OleConnenection)
    ...
    Private Sub AfficheSchemasExtraction()
        Try
            ...
            Dim OledatableSchema As DataTable
            DataListeIntegrer.Rows.Clear()
            OleAdaptaterschema = New OleDbDataAdapter("select * from WED_FORMAT ORDER BY NomFormat ASC", OleConnenection)
            OleSchemaDataset = New DataSet
            OleAdaptaterschema.Fill(OleSchemaDataset)
            OledatableSchema = OleSchemaDataset.Tables(0)
    ...
    Private Sub SelectionFormatMvt_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Me.Name = ""
    End Sub
    Private Sub SelectionFormatMvt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub BT_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_Save.Click
        If Me.Name = "EXTRACTION" Then
            ...
            For Each test In DataListeIntegrer.SelectedRows
                ...
                OleAdaptaterInt = New OleDbDataAdapter("select * from WED_FORMAT Where Chemin='" & Trim(test.Cells("Dossier").Value) & "'", OleConnenection)
                ...
                If OledatableInt.Rows.Count <> 0 Then
                    Schematextractiondocument.DataListeSchema.Rows(Idexe).Cells("NomFormat").Value = Trim(OledatableInt.Rows(0).Item("NomFormat"))
                    Schematextractiondocument.DataListeSchema.Rows(Idexe).Cells("TypeFormat").Value = Trim(OledatableInt.Rows(0).Item("Type"))
                    Schematextractiondocument.DataListeSchema.Rows(Idexe).Cells("Chemin").Value = Trim(OledatableInt.Rows(0).Item("Chemin"))
                    Me.Close()
                End If
            Next
        Else
            ...
            For Each test In DataListeIntegrer.SelectedRows
                ...
                Dim OleIntDataset As DataSet
                Dim OledatableInt As DataTable
                OleAdaptaterInt = New OleDbDataAdapter("select * from FORMATIMPMOUV Where Chemin='" & Trim(test.Cells("Dossier").Value) & "'", OleConnenection)
                OleIntDataset = New DataSet
                OleAdaptaterInt.Fill(OleIntDataset)
                OledatableInt = OleIntDataset.Tables(0)
                If OledatableInt.Rows.Count <> 0 Then
                    SchematintegrerMvt.DataListeSchema.Rows(Idexe).Cells("NomFormat").Value = Trim(OledatableInt.Rows(0).Item("NomFormat"))
                    SchematintegrerMvt.DataListeSchema.Rows(Idexe).Cells("TypeFormat").Value = Afficheauuser(Trim(OledatableInt.Rows(0).Item("Type")))
                    SchematintegrerMvt.DataListeSchema.Rows(Idexe).Cells("Chemin").Value = Trim(OledatableInt.Rows(0).Item("Chemin"))
                    SchematintegrerMvt.DataListeSchema.Rows(Idexe).Cells("Mode").Value = AffichFormatModifiable(Trim(OledatableInt.Rows(0).Item("Chemin")))
                    Me.Close()
                End If
            Next
        End If
    ...
    Private Sub DataListeIntegrer_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataListeIntegrer.CellDoubleClick
        If e.RowIndex >= 0 Then
            If Me.Name = "EXTRACTION" Then
                ...
                Dim OleIntDataset As DataSet
                Dim OledatableInt As DataTable
                OleAdaptaterInt = New OleDbDataAdapter("select * from WED_FORMAT Where Chemin='" & Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("Dossier").Value) & "' ", OleConnenection)
                OleIntDataset = New DataSet
                OleAdaptaterInt.Fill(OleIntDataset)
                OledatableInt = OleIntDataset.Tables(0)
                If OledatableInt.Rows.Count <> 0 Then
            ...
            Else
                ...
                Dim OleIntDataset As DataSet
                Dim OledatableInt As DataTable
                OleAdaptaterInt = New OleDbDataAdapter("select * from FORMATIMPMOUV Where Chemin='" & Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("Dossier").Value) & "' ", OleConnenection)
                OleIntDataset = New DataSet
                OleAdaptaterInt.Fill(OleIntDataset)
                OledatableInt = OleIntDataset.Tables(0)
                If OledatableInt.Rows.Count <> 0 Then

...
Public Class SchematintegrerEcriture
    ...
    Private Sub AfficheSchemasIntegrer()
        ...
        Try
            OleAdaptaterschema = New OleDbDataAdapter("select * from SCHEMASIE", OleConnenection)
            ...
            For i = 0 To OledatableSchema.Rows.Count - 1
                ...
                DataListeIntegrer.Rows(i).Cells("CreationAuto1").Value = OledatableSchema.Rows(i).Item("CreationAuto")
            Next i
        Catch ex As Exception
        ...
        Try
            For i = 0 To DataListeIntegrer.RowCount - 1
                If DataListeIntegrer.Rows(i).Cells("Supprimer").Value = True Then
                    OleAdaptaterDelete = New OleDbDataAdapter("select * from SCHEMASIE where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & "", OleConnenection)
                    ...
                    If OledatableDelete.Rows.Count <> 0 Then
                        DelFile = "Delete From SCHEMASIE where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        DelFile = "Delete From PLANIFICATION where (Intitule='Import Ecriture') and IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & ""
                        OleCommandDelete = New OleDbCommand(DelFile)
                        OleCommandDelete.Connection = OleConnenection
                        OleCommandDelete.ExecuteNonQuery()
                        ...
                        OleAdaptaterEnreg = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT ECRITURE'", OleConnenection)
                        ...
                        If OledatableEnreg.Rows.Count <> 0 Then
                            If Directory.Exists(OledatableEnreg.Rows(0).Item("Repertoire")) = True Then
                                DelFile = "Delete From FTPREPERTOIRE where IDDossier=" & CInt(DataListeIntegrer.Rows(i).Cells("IDDossier1").Value) & " And  Traitement='IMPORT ECRITURE'"
                                OleCommandDelete = New OleDbCommand(DelFile)
                                OleCommandDelete.Connection = OleConnenection
                                OleCommandDelete.ExecuteNonQuery()
                ...
                For n = 0 To DataListeSchema.RowCount - 1
                    If DataListeSchema.Rows(n).Cells("Cible").Value = "Repertoire" Then
                        If IsNumeric(DataListeSchema.Rows(n).Cells("IDDossier").Value) = True Then
                            OleAdaptaterEnreg = New OleDbDataAdapter("select * From SCHEMASIE WHERE Cible='" & DataListeSchema.Rows(n).Cells("Cible").Value & "' and CheminFormat='" & DataListeSchema.Rows(n).Cells("Chemin").Value & "' and CheminFilexport='" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "' and Type='" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "'", OleConnenection)
                            OleEnregDataset = New DataSet
                            OleAdaptaterEnreg.Fill(OleEnregDataset)
                            OledatableEnreg = OleEnregDataset.Tables(0)
                            If OledatableEnreg.Rows.Count <> 0 Then
                            Else
                                OleAdaptaterEnreg = New OleDbDataAdapter("select * From SCHEMASIE WHERE IDDossier=" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & " ", OleConnenection)
                                OleEnregDataset = New DataSet
                                OleAdaptaterEnreg.Fill(OleEnregDataset)
                                OledatableEnreg = OleEnregDataset.Tables(0)
                                If OledatableEnreg.Rows.Count <> 0 Then
                                ...
                                Else
                                    If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                        Insertion = "Insert Into SCHEMASIE (BaseCpta,Cible,CheminFormat,CheminFilexport,NomFormat,NomFilexport,Type,Feuille_Excel,Deplace,IDDossier,CreationAuto) VALUES ('" & DataListeSchema.Rows(n).Cells("BaseCpta").Value & "','" & DataListeSchema.Rows(n).Cells("Cible").Value & "','" & DataListeSchema.Rows(n).Cells("Chemin").Value & "','" & DataListeSchema.Rows(n).Cells("CheminExport").Value & "','" & DataListeSchema.Rows(n).Cells("NomFormat").Value & "','" & DataListeSchema.Rows(n).Cells("DossierExport").Value & "','" & Renvoietypeformat(DataListeSchema.Rows(n).Cells("TypeFormat").Value) & "','" & DataListeSchema.Rows(n).Cells("FeuilleExcel").Value & "'," & DataListeSchema.Rows(n).Cells("Deplace").Value & "," & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & "," & DataListeSchema.Rows(n).Cells("CreationAuto").Value & ")"
                                    ...
                                    Else
                                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                            If RetourneDirectoryFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetournePassWordFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneServeurFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneUserFtp(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" Then
                                                ...
                                                OleCommandEnreg.ExecuteNonQuery()
                                                Insert = True
                                                If Directory.Exists(PatchImportftp & "ECRITURE" & DataListeSchema.Rows(n).Cells("IDDossier").Value) = False Then
                                                    Directory.CreateDirectory(PatchImportftp & "ECRITURE" & DataListeSchema.Rows(n).Cells("IDDossier").Value)
                                                    Insertion = "Insert Into FTPREPERTOIRE (IDDossier,Traitement,Repertoire) VALUES (" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & ",'IMPORT ECRITURE','" & PatchImportftp & "ECRITURE" & DataListeSchema.Rows(n).Cells("IDDossier").Value & "')"
                                                    OleCommandEnreg = New OleDbCommand(Insertion)
                                                    OleCommandEnreg.Connection = OleConnenection
                                                    OleCommandEnreg.ExecuteNonQuery()
                                                Else
                                                    ...
                                                    OleAdaptaterFtp = New OleDbDataAdapter("select * From FTPREPERTOIRE WHERE IDDossier=" & CInt(DataListeSchema.Rows(n).Cells("IDDossier").Value) & " And  Traitement='IMPORT ECRITURE'", OleConnenection)
                                                ...
                                                End If
                                            Else
                                                MsgBox("Impossible d'extraire un paramètre Ftp : " & Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) & " Respectez le format de Saisie !", MsgBoxStyle.Information, "Insertion des Schemas d'integration")
                                            End If
                                        End If
                                    End If
                                End If
                                    ...
                                    Else
                                        If Trim(DataListeSchema.Rows(n).Cells("Chemin").Value) <> "" And Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) <> "" And DataListeSchema.Rows(n).Cells("TypeFormat").Value <> "" Then
                                            If RetourneServeurSQL(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneBaseSQL(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" And RetourneTableSQL(Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value)) <> "" Then
                                                ...
                                                Insert = True
                                            Else
                                                MsgBox("Impossible d'extraire un paramètre SQL Server/Serveur inexistant : " & Trim(DataListeSchema.Rows(n).Cells("CheminExport").Value) & " Respectez le format de Saisie !", MsgBoxStyle.Information, "Insertion des Schemas d'integration")
                                            End If
                                        End If
                                    End If
                                Else
    ...
    Private Sub BT_DelRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_DelRow.Click
        ...
        For i As Integer = 0 To DataListeSchema.RowCount - 1
            DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("SCHEMASIE") + i
        Next i
    End Sub
    Private Sub BT_ADD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_ADD.Click
        ...
        DataListeSchema.Rows(i).Cells("CreationAuto").Value = False
        DataListeSchema.Rows(i).Cells("IDDossier").Value = RenvoieID("SCHEMASIE") + i
        If RenvoieID("SCHEMASIE") = DataListeSchema.Rows(0).Cells("IDDossier").Value Then

        Else
            For j As Integer = 0 To DataListeSchema.RowCount - 1
                DataListeSchema.Rows(j).Cells("IDDossier").Value = RenvoieID("SCHEMASIE") + j
            Next j
        End If
    End Sub
        ...
        Try
            If e.RowIndex >= 0 Then
                If DataListeSchema.Columns(e.ColumnIndex).Name = "Type" Then
                    Idexe = e.RowIndex
                    SelectionFormatEcriture.ShowDialog()
                End If
                    ...
                    ElseIf DataListeSchema.Rows(e.RowIndex).Cells("Cible").Value = "BaseSQL" Then
                        If BaseSQLConnexion(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)), RetourneBaseSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)), RetourneUserSQL(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value))), RetournePasseSQL(RetourneServeurSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)))) = True Then
                            If ExisteTableSQL(RetourneTableSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value))) = True Then
                                SelectionFiltre.Name = "SelectionFiltreEC"
                            ...
                            Else
                                MsgBox("Connexion à la Base SQL : " & RetourneBaseSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)) & " Réussie !" & Chr(13) & "" & Chr(13) & "La table SQL : " & RetourneTableSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)) & " n'existe pas dans la base!")
                            End If
                        Else
                            MsgBox("Echec de Connexion à la Base SQL : " & RetourneBaseSQL(Trim(DataListeSchema.Rows(e.RowIndex).Cells("CheminExport").Value)) & " Vérifiez les paramètres de Connexion !")
                        End If
                    End If
                End If
    ...
    Private Sub MiseàjourLeSchema()
        ...
        If DataListeIntegrer.RowCount >= 0 Then
            For n = 0 To DataListeIntegrer.RowCount - 1
                OleAdaptaterEnreg = New OleDbDataAdapter("select * From SCHEMASIE where  IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & "", OleConnenection)
                OleEnregDataset = New DataSet
                OleAdaptaterEnreg.Fill(OleEnregDataset)
                OledatableEnreg = OleEnregDataset.Tables(0)
                If OledatableEnreg.Rows.Count <> 0 Then
                    If DataListeIntegrer.Rows(n).Cells("Deplace1").Value = False Then
                        Insertion = "UPDATE SCHEMASIE SET CreationAuto=" & DataListeIntegrer.Rows(n).Cells("CreationAuto1").Value & ",Feuille_Excel='" & Join(Split(DataListeIntegrer.Rows(n).Cells("FeuilleExcel1").Value, "'"), "''") & "',Deplace=False where  IDDossier=" & CInt(DataListeIntegrer.Rows(n).Cells("IDDossier1").Value) & ""
                        OleCommandEnreg = New OleDbCommand(Insertion)
                        OleCommandEnreg.Connection = OleConnenection
                        OleCommandEnreg.ExecuteNonQuery()
                        Insert = True
                    Else
                        Insertion = "UPDATE SCHEMASIE SET CreationAuto=" & DataListeIntegrer.Rows(n).Cells("CreationAuto1").Value & ",Feuille_Excel='" & Join(Split(DataListeIntegrer.Rows(n).Cells("FeuilleExcel1").Value, "'"), "''") & "',Deplace=True where Cible='" & DataListeIntegrer.Rows(n).Cells("Cible1").Value & "' And  CheminFormat='" & DataListeIntegrer.Rows(n).Cells("CheminForma").Value & "' and CheminFilexport='" & DataListeIntegrer.Rows(n).Cells("CheminRepexpor").Value & "' and Type='" & Renvoietypeformat(DataListeIntegrer.Rows(n).Cells("TypeFormat1").Value) & "'"
                        OleCommandEnreg = New OleDbCommand(Insertion)
                        OleCommandEnreg.Connection = OleConnenection
                        OleCommandEnreg.ExecuteNonQuery()
                        Insert = True
            ...
            Next n
            If Insert = True Then
                MsgBox("Mise à Jour Reussie", MsgBoxStyle.Information, "Mise à Jour des Schemas d'integration")
            End If
        End If
        AfficheSchemasIntegrer()
    End Sub
                    ...
                    ElseIf DataListeIntegrer.Rows(e.RowIndex).Cells("Cible1").Value = "BaseSQL" Then
                        If BaseSQLConnexion(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)), RetourneBaseSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)), RetourneUserSQL(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value))), RetournePasseSQL(RetourneServeurSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)))) = True Then
                            If ExisteTableSQL(RetourneTableSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value))) = True Then
                                SelectionFiltre.Name = "SelectionFiltreEC1"
                                SelectionFiltre.Txttable.Text = RetourneTableSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value))
                                Idexe = e.RowIndex
                                SelectionFiltre.Show()
                            Else
                                MsgBox("Connexion à la Base SQL : " & RetourneBaseSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)) & " Réussie !" & Chr(13) & "" & Chr(13) & "La table SQL : " & RetourneTableSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)) & " n'existe pas dans la base!")
                            End If
                        Else
                            MsgBox("Echec de Connexion à la Base SQL : " & RetourneBaseSQL(Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("CheminRepexpor").Value)) & " Vérifiez les paramètres de Connexion !")
                        End If
                    End If
                End If
            End If

...
Public Class SelectionFormatStock
    ...
    Private Sub AfficheSchemasIntegration()
        Try
            ...
            OleAdaptaterschema = New OleDbDataAdapter("select * from WIS_FORMAT ORDER BY NomFormat ASC", OleConnenection)
    ...
    Private Sub BT_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_Save.Click
        ...
        For Each test In DataListeIntegrer.SelectedRows
            ...
            OleAdaptaterInt = New OleDbDataAdapter("select * from WIS_FORMAT Where Chemin='" & Trim(test.Cells("Dossier").Value) & "'", OleConnenection)
            ...
            If OledatableInt.Rows.Count <> 0 Then
                SchematintegrerStock.DataListeSchema.Rows(Idexe).Cells("NomFormat").Value = Trim(OledatableInt.Rows(0).Item("NomFormat"))
                SchematintegrerStock.DataListeSchema.Rows(Idexe).Cells("TypeFormat").Value = Afficheauuser(Trim(OledatableInt.Rows(0).Item("Type")))
                SchematintegrerStock.DataListeSchema.Rows(Idexe).Cells("Chemin").Value = Trim(OledatableInt.Rows(0).Item("Chemin"))
                SchematintegrerStock.DataListeSchema.Rows(Idexe).Cells("Mode").Value = AffichFormatModifiable(Trim(OledatableInt.Rows(0).Item("Chemin")))
                Me.Close()
            End If
        Next
    End Sub
    Private Sub DataListeIntegrer_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataListeIntegrer.CellDoubleClick
        If e.RowIndex >= 0 Then
            ...
            Dim OleIntDataset As DataSet
            Dim OledatableInt As DataTable
            OleAdaptaterInt = New OleDbDataAdapter("select * from WIS_FORMAT Where Chemin='" & Trim(DataListeIntegrer.Rows(e.RowIndex).Cells("Dossier").Value) & "' ", OleConnenection)
            OleIntDataset = New DataSet
            OleAdaptaterInt.Fill(OleIntDataset)
            OledatableInt = OleIntDataset.Tables(0)
            If OledatableInt.Rows.Count <> 0 Then
